...
function getJokeHTML(joke){
    return `<div class="joke" id="joke_${joke.id}">
...
function like(id){
    ...
    likeXhr.open('GET', 'http://localhost:3000/like?id=');
    likeXhr.send();
    likeXhr.responseType = 'json';
    likeXhr.onload = () => {
        const joke = likeXhr.response;
        document.getElementById('joke_'+id).outerHTML = gerJokeHtml(joke);
    };
}

function dislike(id){
    const dislikeXhr = new XMLHttpRequest();
    dislikeXhr.open('GET', 'http://localhost:3000/dislike?id=');
    dislikeXhr.send();
